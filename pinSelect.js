...
.directive('pinSelect', function (){
    ...
    $.fn.autotab_filter = function (options) {
        ...
        return $(this).autotab('filter', defaults);
    	  ...
    	  angular.element('.number').autotab('number');
    	  
    	  angular.element('.input-last').keyup(function(){
            if(this.value.length==$(this).attr("maxlength")){
                 if ($scope.pin1 && $scope.pin2 && $scope.pin3) {
                    debugger;
                $scope.pin = $scope.pin1 + $scope.pin2 + $scope.pin3 + this.value;
                $scope.$apply();
      ...
      /* $scope.dateChanged = function () {
          if ($scope.pin1 && $scope.pin2 && $scope.pin3 && $scope.pin4) {
            $scope.pin = $scope.pin1 + $scope.pin2 + $scope.pin3 + $scope.pin4;
